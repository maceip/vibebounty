...
def parse_user(content: str) -> dict:
    """Parse the canonical _render() user turn back into a submission dict.

    Matches app/triage.py::_render. Stops at the `---` divider: the corroboration
    block after it is regenerated by triage.run via enrich(), so we must not fold
    it into any field.
    """
    sub = {"title": "", "severity_claimed": "", "asset": "",
           "description": "", "steps_to_reproduce": "", "impact": ""}
    buckets = {"description": [], "steps_to_reproduce": [], "impact": []}
    section = None
